...
BEGIN
	...
	BEGIN TRY
		...
		******************************************************************/	
		IF OBJECT_ID('DBO.tmp_final_allocation', 'U') IS NOT NULL
		  DROP TABLE DBO.tmp_final_allocation;

		select distinct a.AuditversionID	
			,DT.int_iso_yearweek AS ETD_WEEK
			,a.voyage	
			,a.VesselCode	
			,a.servicecode	
		...
		******************************************************************/	
		...
		(
			...
			left join (
				...
				from DBO.TMP_capinv_commited_legs
			...
			LEFT JOIN DBO.tmp_final_allocation NG
			ON NT.int_iso_yearweek = NG.ETD_WEEK
				AND NT.VESSEL = NG.VESSELCODE
		   ...
		   ,MK.[ISC]
			...
			(
				SELECT distinct 
						...
						,A.VESSELCODE	
						,A.SERVICECODE	
						,COALESCE(CON.LEGSEQID, A.LEGSEQID) AS LEGSEQID
						,COALESCE(CON.SERVICECODEDIRECTION,A.SERVICECODEDIRECTION) as SERVICECODEDIRECTION
						,A.DEPARTUREPORT	
						,A.ARRIVALPORT
						--,A.VESSELOPERATOR	
						,COALESCE(CON.DEPARTUREDATE, A.DEPARTUREDATE) AS DEPARTUREDATE	
				...
				FROM  dbo.tmp_final_allocation A
				...
				AND A.DEPARTUREPORT = B.DEPARTUREPORT
				AND A.ARRIVALPORT = B.ARRIVALPORT
				LEFT JOIN DBO.TMP_AGG_CAPFCST_FREESALECONSUMPTION CON
				...
				AND A.ISC = CON.ISC
				WHERE A.SERVICECODE IN (SELECT DISTINCT SERVICECODE FROM DBO.TMP_FINAL_ALLOCATION)
		)MK;
