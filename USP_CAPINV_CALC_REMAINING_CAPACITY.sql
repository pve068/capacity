	...
	BEGIN TRY
		...
		******************************************************************/	
		...
		EXEC DBO.USP_LOG_PROCESS_DETAIL @V_PROCESS_ID = @V_PROCESS_ID
				...
				FROM ANALYTICSDATAMART.DBO.VW_FREESALEBOOKING -- TODO FILTER SO WE ONLY HAVE FREESALE SHIPMENTS FROM VW_FREESALEBOOKING
				...
				LEFT JOIN
				ANALYTICSDATAMART.DBO.ROUTELINKS B
		  ...
		  DROP TABLE CAPINVREPOSITORY.DBO.TMP_FINAL_ALLOCATION_OVERBOOK
			...
			(
				 SELECT DISTINCT A.VOYAGE	
								...
								,CASE WHEN B.TEU_OTB >= 0.5*A.CAL_ALLOCATIONTEU THEN WGTD_AVG_MATL_PROB	
										ELSE C.MATL_PROB 
								END MATL_PROB -- TODO PARAMETER HARD CODED HERE
								,(CASE WHEN B.TEU_OTB >= 0.5*A.CAL_ALLOCATIONTEU THEN WGTD_AVG_MATL_PROB ELSE C.MATL_PROB END) as ded
				FROM CAPINVREPOSITORY.DBO.TMP_FINAL_ALLOCATION A -- TODO FILTER SO WE ONLY GET PUBLISHED RUN ID, TODO ALSO WE NEED TO DO THIS QUERY UPSTREAM OF THIS TABLE BECAUSE WE WANT THE RESULTS PUBLISHED IN THIS TABLE
			...
			SELECT DISTINCT A.VOYAGE	
								...
								,A.MATL_PROB -- TODO PARAMETER HARD CODED HERE
								,case when a.ded = 0.0 then 0.0
									else ISNULL(1.0/
													(CASE WHEN A.TEU_OTB >= 0.5*A.CAL_ALLOCATIONTEU THEN A.WGTD_AVG_MATL_PROB ELSE A.MATL_PROB END)-1,0.0) 
								end as OVERBOOK_TEU_PCT -- TODO PARAMETER HARD CODED HERE
		...
		****************************************************************/	
		...
		EXEC DBO.USP_LOG_PROCESS_DETAIL @V_PROCESS_ID = @V_PROCESS_ID
					...
					select  distinct dat.SCHEDULEID	
							,dat.SERVICECODE	
							,dat.VESSELCODE	
							,dat.VOYAGE	
							,dat.ARRVOYAGE	
							,dat.LEGSEQID	
							,dat.DEPARTUREPORT	
							,dat.serviceCodeDirection	
							,dat.ARRIVALPORT	
							,dat.ORIGINAL_ETD as DEPARTUREDATE
							,dat.ARRIVALDATE
							,dat.VESSELOPERATOR
							,isnull(ds.ISC,'LEAD') as ISC
							,ds.INTAKETEU	
							,ds.INTAKETONS	
							,ds.INTAKEPLUGS	
							,ds.CAL_ALLOCATIONTEU	
							,ds.CAL_ALLOCATIONTONS	
							,ds.CAL_ALLOCATIONPLUGS	
							,ds.ISC_ALLOCATIONTEU	
							,ds.ISC_ALLOCATIONTONS	
							,ds.ISC_ALLOCATIONPLUGS	
					into #CAPINV_COMBINATIONS
					from ANALYTICSDATAMART.DBO.ROUTELEGS_SCHEDULE dat
					left join CAPINVREPOSITORY.DBO.TMP_FINAL_ALLOCATION_OVERBOOK ds
					ON dat.VESSELCODE = ds.VESSELcode
					AND dat.VOYAGE = ds.VOYAGE
					AND dat.SERVICECODE = ds.SERVICECODE
					AND dat.DEPARTUREPORT = ds.DEPARTUREPORT
					AND dat.ARRIVALPORT = ds.ARRIVALPORT
					WHERE cast(dat.ORIGINAL_ETD as date) BETWEEN DATEADD(DAY, -@V_LOOKBACK_DAYS, @V_RUNDATE) AND DATEADD(DAY, @V_LOOKFORWARD_DAYS,@V_RUNDATE)
