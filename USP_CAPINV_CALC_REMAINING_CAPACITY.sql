	...
	BEGIN TRY
		...
		******************************************************/
		...
		SELECT A.WEEKNO,A.VESSELCODE AS VESSEL,A.VOYAGE
				...
				,SUM(A.WEIGHTKG * COALESCE(B.TEU_DIST,1))/@V_KG_TO_TONS AS CAL_MTS
		...
		******************************************************************/	
		...
		(
			...
			(
				SELECT weekno,VESSEL,VOYAGE,SERVICECODE,DEPARTUREPORT,ARRIVALPORT
				...
				GROUP BY weekno,VESSEL,VOYAGE,SERVICECODE,DEPARTUREPORT,ARRIVALPORT
			) B
		  ...
		  SELECT DISTINCT VOYAGE	
			...
			FROM DATA_TMP A
			WHERE DEPARTUREDATE BETWEEN DATEADD(DAY, -@V_LOOKBACK_DAYS, @V_RUNDATE) AND DATEADD(DAY, @V_LOOKFORWARD_DAYS,@V_RUNDATE);
		...
		******************************************************************/	
		...
		(
			...
			WHERE NT.ETD BETWEEN DATEADD(DAY, -@V_LOOKBACK_DAYS, @V_RUNDATE) AND DATEADD(DAY, @V_LOOKFORWARD_DAYS,@V_RUNDATE)
			GROUP BY	NT.VESSEL
						,NT.VOYAGE
						,NT.SERVICECODE
						, NT.FROMLEGSITECODE
		   ...
		   ,MK.[ISC]
			...
			(
				SELECT DISTINCT 
						...
						, COALESCE(CON.COMMITMENT_CON_TEU,0) AS TOTALCOMMITFILINGSTEU
						--FREESALE Available --WITH NO OVERBOOK		
						, A.CAL_ALLOCATIONTEU 
							- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END 
							- COALESCE(CON.COMMITMENT_CON_TEU,0) AS FREESALEALLOCATIONTEU
						--Over booked teu = Overbook percent * (FREESALE Available --WITH NO OVERBOOK)
						, ROUND(((0.0) * (A.CAL_ALLOCATIONTEU 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TEU,0))),0) AS OVERBOOKTEU
						--Overbook Percent
						...
						, ( A.CAL_ALLOCATIONTEU 
							- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END 
							- COALESCE(CON.COMMITMENT_CON_TEU,0))
							 + ROUND(( (0.0)* ( A.CAL_ALLOCATIONTEU 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TEU,0))),0) AS FREESALE_AVAILABLETEU
						...
						, (( A.CAL_ALLOCATIONTEU 
							 ...
							 + ROUND(( (0.0)* (A.CAL_ALLOCATIONTEU 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TEU,0))),0) ) 
								- COALESCE(CON.FS_CONSUMPTIONTEU,0) 
						...
						, ((( A.CAL_ALLOCATIONTEU 
							...
							- COALESCE(CON.COMMITMENT_CON_TEU,0))
							 + ROUND(( (0.0)* ( A.CAL_ALLOCATIONTEU 
									- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTEU,0) ELSE 0 END
									- COALESCE(CON.COMMITMENT_CON_TEU,0))),0) ) - COALESCE(CON.FS_CONSUMPTIONTEU,0)) / @V_FFE_TEU_MULTIPLIER AS REMAINING_FREESALEFFE
						--EMPTIES AND TONS
						, CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END AS TOTALEMPTYTONS
						--Commitment Filings --Tons
						, COALESCE(CON.COMMITMENT_CON_TONS,0) AS TOTALCOMMITFILINGSTONS
						--FREE SALES ALLOCATION TONS 
						, A.CAL_ALLOCATIONTONS 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TONS,0) AS FREESALEALLOCATIONTONS
						,((0.0)* ( A.CAL_ALLOCATIONTONS 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TONS,0))) AS OVERBOOKTONS
						,(A.CAL_ALLOCATIONTONS 
							- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END 
							- COALESCE(CON.COMMITMENT_CON_TONS,0))
							 + ((0.0)* (A.CAL_ALLOCATIONTONS 
								- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END 
								- COALESCE(CON.COMMITMENT_CON_TONS,0))) AS FREESALE_AVAILABLETONS
						--AVAILABLE TONS -- NO OVERBOOKING
						, (A.CAL_ALLOCATIONTONS 
							- CASE WHEN A.ISC = 'LEAD' THEN COALESCE(B.TOTALEMPTYTONS,0) ELSE 0 END 
							- COALESCE(CON.COMMITMENT_CON_TONS,0))  AS FREESALE_AVAILABLENOOVERBOOKTONS
						...
						,COALESCE(CON.COMMITMENT_CON_PLUGS,0) AS TOTALCOMMITFILINGSPLUGS
						,A.CAL_ALLOCATIONPLUGS 
							- COALESCE(CON.COMMITMENT_CON_PLUGS,0) AS FREESALEALLOCATIONPLUGS
						,ROUND(((0.0)* (A.CAL_ALLOCATIONPLUGS 
							- COALESCE(CON.COMMITMENT_CON_PLUGS,0))),0) AS OVERBOOKPLUGS
						,(A.CAL_ALLOCATIONPLUGS 
							- COALESCE(CON.COMMITMENT_CON_PLUGS,0))
							+ ROUND(((0.0)* (A.CAL_ALLOCATIONPLUGS 
									- COALESCE(CON.COMMITMENT_CON_PLUGS,0))),0) AS FREESALE_AVAILABLEPLUGS
						--AVAILABLE PLUGS - NO OVERBOOKING
						,( A.CAL_ALLOCATIONPLUGS 
							- COALESCE(CON.COMMITMENT_CON_PLUGS,0)) AS FREESALE_AVAILABLENOOVERBOOKPLUGS
						, COALESCE(CON.FS_CONSUMPTIONPLUGS,0) AS FREESALECONSUMPTIONPLUGS
						, ROUND((( A.CAL_ALLOCATIONPLUGS 
							 - COALESCE(CON.COMMITMENT_CON_PLUGS,0))
							 + ROUND(((0.0)* (A.CAL_ALLOCATIONPLUGS 
									- COALESCE(CON.COMMITMENT_CON_PLUGS,0))),0) ) 
				...
				FROM  CAPINVREPOSITORY.DBO.TMP_FINAL_ALLOCATION A (NOLOCK)
