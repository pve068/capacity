...
/****** Object:  StoredProcedure [dbo].[USP_CAPINV_CALC_REMAINING_CAPACITY]    Script Date: 12-02-2018 11:12:04 ******/
SET ANSI_NULLS ON
GO
...
* Modification History:Replaced STRINGS with Routes as per the new ISC source structure
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC DBO.USP_LOG_PROCESS_DETAIL @V_PROCESS_ID = @V_PROCESS_ID
			...
			/******************************************************
			BUILD HISTORICAL DISTRIBUTION TABLE	AT THE 
			SERVICECODE/DIRECTIO/STRING CODE/ ROUTE CODE LEVEL
			*******************************************************/
		...
		EXEC DBO.USP_LOG_PROCESS_DETAIL @V_PROCESS_ID = @V_PROCESS_ID
			...
			CREATE CLUSTERED INDEX CDX_DATA_2 ON DBO.TMP_ISC_LIST_AVG(SERVICECODE,ROUTE_CD)
		...
		******************************************************/
		...
		/*****************************************************************
				GET CALCULATED ALLOCATION BY SUBTRACTING THE ISC STREED
				COMMITED ALLOCATION FROM THE COMS LEAD ALLOCATION. THIS
				IS THE ALLOCATION THAT WILL BE USED FOR THE UI.
				NOTE: IF THERE ARE ALLOCATION ISSUES CHECK BOTH THESE 
				ALLOCATION NUMBERS
		******************************************************************/	
		...
		(
			...
			(
				SELECT --weekno,
						VESSEL,VOYAGE,SERVICECODE,SERVICECODEDIRECTION,DEPARTUREPORT,ARRIVALPORT
		...
		*************************************************************/
		...
		/**************************************************************
			CALCULATE FREESALE AND COMMITMENT CONSUMBTION BY 
			AGGREGATING THE CAPFCST_AD_CONSUMPTION TABLE AT
			THE VESSEL/VOYAGE/SERVICE/DEPARTURE PORT/ARRIVAL PORT
			LEVELS.
		*************************************************************/
		...
		(
			SELECT  NT.VESSEL
					...
					,CAST(CASE WHEN NN.BUYINGROUTE IS NULL OR (NN.BUYINGROUTE IS NOT NULL AND LD.ROUTE_CD IS NULL) THEN 'LEAD'
						   ELSE NT.ROUTE_CD 
			...
			LEFT JOIN (
				SELECT DISTINCT SERVICECODE,BUYINGROUTE,VESSELCODE,VOYAGE,DEPARTUREPORT,ARRIVALPORT
				FROM ANALYTICSDATAMART.DBO.AD_ISC_COMMITED_LEGS
				WHERE BUYINGROUTE IS NOT NULL AND BUYINGROUTE <> ''
			) NN
			ON 
			
			NT.SERVICECODE = NN.SERVICECODE
		...
		/***************************************************************
			GET COMMITMENT NUMBERS. THIS TABLE IS PROVIDED BY THE 
			COMMITMENT TEAM AND SHOULD BE REFRESHED DAILY
		****************************************************************/
		SET @V_STEP_ID = 100
