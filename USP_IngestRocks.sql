...
/****** Object:  StoredProcedure [dbo].[USP_IngestRocks]    Script Date: 7/21/2017 2:19:00 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[USP_IngestRocks]
/****************************************************************************************************
* Copyright REVENUE ANALYTICS 2017
* All Rights Reserved
*
* Created By: DATA ENGINEERING
* FileName:   USP_IngestRocks.SQL
* Date:       04/07/2017
*
...
* Input(s):		stg_rock
*				dim_rock_container
* Output(s):	AnalyticsDataMart.dbo.AD_Rocks
*
* Assumptions: ETLReopository.dbo.stg_rocks table is updated
*
* Example Call: exec USP_IngestRocks 0, @V_ERROR_CODE OUTPUT
*
*
*****************************************************************************************************
* Notes:
...
BEGIN
	...
	BEGIN TRY
		...
		SET @V_NOTE = 'Delete existing Rocks data'
		...
		*******************************************************************/
		...
		SET @V_NOTE = 'Refresh the Empties (ROCKS) table'
		EXEC dbo.USP_LOG_PROCESS_DETAIL @V_PROCESS_ID = @V_PROCESS_ID
  					, @V_STEP_ID = @V_STEP_ID
  					, @V_STEP_NAME = @V_STEP_NAME
  					, @V_START_DT = @V_SYSTEMTIME
		...
		LEFT JOIN AnalyticsDataMart.dbo.dim_rock_container b
